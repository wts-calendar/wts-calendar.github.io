...
import { ApiClientReference } from './api-client-reference';

type ReferenceKind = 'options' | 'methods' | 'events' | 'symbols' | 'server';

@Component({
  selector: 'app-api-reference-page',
  imports: [RouterLink, ApiClientReference],
  template: `
    @if (kind() !== 'server') {
      <app-api-client-reference [kind]="kind()" />
    } @else {
      <section class="page-heading container api-reference-heading">
        ...
        <p>
          Search the installed <code>@wts-calendar/core</code> declarations, public calendar
          methods, and every PHP and ASP.NET Core endpoint setting. Types and method signatures are
          generated from the package pinned by this portal.
        </p>
        ...
        @if (kind() === 'server') {
          <section class="api-server-intro">
            ...
            <p>
              ...
              semantics. Their field validation and recurrence-key casing have documented
              differences. PHP supplies a PSR-15 handler; ASP.NET Core supplies Minimal API route
              extensions. Authentication, authorization, tenant isolation, and durable storage stay
              in your application.
            </p>
          ...
          <section class="api-response-section">
            ...
            <div class="api-compatibility-note">
              ...
              <ul>
                ...
                <li>
                  PATCH and PUT both replace the event representation; the route ID wins over any
                  body ID.
                </li>
                <li>
            ...
            <ul>
              <li>
                ...
                <code>IWtsCalendarEventStore</code>; bundled in-memory stores are for demos and
                tests.
              </li>
              <li>
                Apply authentication and event-level authorization before the calendar
                handler/routes.
              </li>
              <li>
